...
const ALL_EXCLUDABLE_TAGS: string[] = ["DEMO", "BETA", "PROTO", "UNL", "AFTERMARKET", "AUTO DEMO", "BIOS", "PROGRAM"];
const ALL_EXCLUDABLE_REGIONS: string[] = ["Europe", "World", "USA", "Spain", "France", "Germany", "Japan", "Korea", "Asia", "China", "Brazil", "Russia", "Taiwan"];
const DEFAULT_LANGUAGES_ORDER: string[] = ["Es", "En", "Fr", "De", "It", "Pt", "Ja"];

interface BooleanStates {
    [key: string]: boolean;
}

const DEFAULT_TAG_EXCL_STATES: BooleanStates = Object.fromEntries(ALL_EXCLUDABLE_TAGS.map(tag => [tag, true]));
const DEFAULT_REGION_EXCL_STATES: BooleanStates = {
...
interface AppConfig {
    exclude_tags: BooleanStates;
    preferred_regions: string[];
    preferred_languages: string[];
    exclude_regions: BooleanStates;
}
...
interface RevisionInfo {
    value: string; // e.g., "A", "1", "B1"
    numericValue?: number; // Parsed number if value is purely numeric
}

interface ParsedRomInfo {
    baseName: string;
    regions: string[];
    languages: string[];
    tags: string[];
    miscTags: string[]; // Should not contain successfully parsed versions/revisions
...
interface SimulacionResultados {
    salvados: string[];
    eliminados: string[];
    mapa_url: { [romName: string]: string };
}

// --- Referencias a Elementos del DOM ---
let tagsCheckboxesContainer: HTMLElement | null, 
    regionesExcluirCheckboxesContainer: HTMLElement | null, 
    rvRegionesContainer: HTMLElement | null, 
    rvIdiomasContainer: HTMLElement | null,
    rvSalvadosContainer: HTMLElement | null, 
    rvEliminadosContainer: HTMLElement | null, 
    btnRegionUp: HTMLElement | null, 
    btnRegionDown: HTMLElement | null, 
    btnIdiomaUp: HTMLElement | null, 
    btnIdiomaDown: HTMLElement | null,
    btnPrevisualizar: HTMLButtonElement | null, 
    btnMoverAEliminados: HTMLElement | null, 
    btnMoverASalvados: HTMLElement | null,
    btnGenerarFinal: HTMLButtonElement | null, 
    fileInput: HTMLInputElement | null, 
    filePathDisplay: HTMLElement | null, 
    labelSalvados: HTMLElement | null, 
    labelEliminados: HTMLElement | null, 
    statusLabel: HTMLElement | null,
    customMessageBox: HTMLElement | null, 
    customMessageText: HTMLElement | null, 
    customMessageButton: HTMLElement | null;
...
let romFileContent: RomFileEntry[] | null = null; 
let simulacionResultados: SimulacionResultados = { salvados: [], eliminados: [], mapa_url: {} };
...
// --- Funciones de Parseo ---
function parseRomName(filename: string): ParsedRomInfo {
    ...
    let isBadDump = false;
    let parsedVersion: VersionInfo | undefined;
    let parsedRevision: RevisionInfo | undefined;
    const collectedMiscTags: string[] = [];
    ...
    const extractedRawItems: string[] = [];
    let matchEnclosed;
    ...
    for (const rawItem of extractedRawItems) {
        ...
        for (const part of parts) {
            ...
            if (!partProcessed) {
                 if (upperPart === "!" || upperPart === "V" || upperPart === "VERIFIED" || upperPart === "GOOD DUMP") { isVerifiedGood = true; partProcessed = true; }
            }
            if (!partProcessed) {
                 if (upperPart === "B" || upperPart === "BAD DUMP" || upperPart === "BAD") { isBadDump = true; partProcessed = true; }
            }
            ...
            if (!partProcessed) {
                ...
                let finalMappedRegion: string | undefined = mappedRegionByShortCode || knownFullRegionMatch;
                if (finalMappedRegion && !regions.includes(finalMappedRegion)) {
                     regions.push(finalMappedRegion); partProcessed = true;
                }
            }
            // Check for Languages
            if (!partProcessed) {
                ...
                let finalMappedLang: string | undefined = mappedLangByShortCode || knownFullLangMatch;
                if (finalMappedLang && !languages.includes(finalMappedLang)) {
                     languages.push(finalMappedLang); partProcessed = true;
                }
            }
            // Check for Excludable Tags
            if (!partProcessed) {
                if (KNOWN_EXCLUDABLE_TAGS_FOR_PARSING.has(upperPart) && !tags.includes(upperPart)) {
                     tags.push(upperPart); partProcessed = true; 
                }
            }
    ...
    if (languages.length === 0) {
        if (regions.some(r => ["USA", "World", "Europe", "UK", "Australia"].includes(r))) languages.push("En");
        else if (regions.includes("Spain")) languages.push("Es");
        else if (regions.includes("Japan")) languages.push("Ja");
        else if (regions.includes("France")) languages.push("Fr");
        else if (regions.includes("Germany")) languages.push("De");
        else if (regions.includes("Italy")) languages.push("It");
    }
    if (languages.length === 0 && regions.length > 0) { 
        languages.push("En"); // Default to English if regions are specified but no language
    }
    
    let baseNameCandidates: string[] = [workName.trim()];
    ...
    if(baseNameCandidates.length > 0) finalBaseName = baseNameCandidates[0];
...
function compareParsedInfo(a: ParsedRomInfo, b: ParsedRomInfo): number {
    ...
    if (a.version && b.version) {
        ...
        if (a.version.sub && b.version.sub) {
            if (a.version.sub < b.version.sub) return 1; // 'b' > 'a' -> b is better
            if (a.version.sub > b.version.sub) return -1; // 'a' > 'b' -> a is better
        } else if (a.version.sub) return -1; // a has sub, b doesn't (a is more specific/later)
        else if (b.version.sub) return 1;  // b has sub, a doesn't
    } else if (a.version) return -1; // a has version, b doesn't
    else if (b.version) return 1;  // b has version, a doesn't
    ...
    if (a.revision && b.revision) {
        if (a.revision.numericValue !== undefined && b.revision.numericValue !== undefined) {
            if (a.revision.numericValue !== b.revision.numericValue) return b.revision.numericValue - a.revision.numericValue;
        } else if (a.revision.numericValue !== undefined) return -1; // a is numeric, b isn't (numeric preferred)
        else if (b.revision.numericValue !== undefined) return 1;  // b is numeric, a isn't
        
        // If both non-numeric (or both numeric and equal), compare string value (e.g., "B" vs "A")
        if (a.revision.value < b.revision.value) return 1; // b's revision value is "greater"
        if (a.revision.value > b.revision.value) return -1; // a's revision value is "greater"
    } else if (a.revision) return -1; // a has revision, b doesn't
    else if (b.revision) return 1;  // b has revision, a doesn't
...
// --- Lógica de Filtrado ---
function simular_filtro_js(
    romEntries: RomFileEntry[],
    config: AppConfig 
): SimulacionResultados {
    const allParsedEntries: Required<RomFileEntry>[] = romEntries.map(entry => ({
        ...entry,
        url: entry.url ?? "", 
        parsedInfo: entry.parsedInfo || parseRomName(entry.name) 
    }));

    const initialEliminatedNames = new Set<string>();
    const candidates: Required<RomFileEntry>[] = [];
    ...
    for (const entry of allParsedEntries) {
        ...
        if (excludedByTag) { initialEliminatedNames.add(entry.name); continue; }

        if (entry.parsedInfo.regions.length > 0) {
            const allMyRegionsAreGloballyExcluded = entry.parsedInfo.regions.every(
                region => config.exclude_regions[region] === true
            );
    ...
    const groupedByBaseName: { [baseName: string]: Required<RomFileEntry>[] } = {};
    ...
    const finalSalvadosNames = new Set<string>();
    const tempMapaUrl: { [romName: string]: string } = {};

    for (const baseName in groupedByBaseName) {
        const originalGroupForBaseName = groupedByBaseName[baseName];
        if (originalGroupForBaseName.length === 0) continue;
        ...
        let regionFilteredCandidates: Required<RomFileEntry>[] = [];
        for (const prefRegion of config.preferred_regions) {
            if (config.exclude_regions[prefRegion] === true) {
                continue; 
            }
            const foundInThisPrefRegion = bestCandidatesThisGroup.filter(e => 
                e.parsedInfo.regions.includes(prefRegion)
            );
        ...
        if (bestCandidatesThisGroup.length === 1) {
             ...
             originalGroupForBaseName.forEach(entry => {
                 if (entry.name !== chosen.name) initialEliminatedNames.add(entry.name);
             });
             continue;
        }

        let langFilteredCandidates: Required<RomFileEntry>[] = [];
        ...
        if (bestCandidatesThisGroup.length > 1) {
            bestCandidatesThisGroup.sort((a, b) => {
                ...
                if (a.parsedInfo.isVerifiedGood && !b.parsedInfo.isVerifiedGood) return -1;
                if (!a.parsedInfo.isVerifiedGood && b.parsedInfo.isVerifiedGood) return 1;
                // 2. Bad Dump flag (less bad is better)
                if (!a.parsedInfo.isBadDump && b.parsedInfo.isBadDump) return -1;
                if (a.parsedInfo.isBadDump && !b.parsedInfo.isBadDump) return 1;
                ...
                if (!aHasCurrentlyExcludedTag && bHasCurrentlyExcludedTag) return -1;
                if (aHasCurrentlyExcludedTag && !bHasCurrentlyExcludedTag) return 1;

                // 4. Version/Revision comparison (new)
                const versionRevisionComparison = compareParsedInfo(a.parsedInfo, b.parsedInfo);
                if (versionRevisionComparison !== 0) return versionRevisionComparison;
    ...
    const finalEliminadosNames = new Set<string>(initialEliminatedNames);
...
function populateCheckboxes(container: HTMLElement | null, items: string[], states: BooleanStates, groupName: string, containerIdForLog: string): void {
    if (!container) {
        console.error(`populateCheckboxes ('${groupName}'): CRITICAL - Contenedor (ID: ${containerIdForLog}) NO ENCONTRADO.`);
        if (statusLabel) updateStatus(`Error: Contenedor para '${groupName}' no encontrado.`);
        return;
    }
    container.innerHTML = ''; 
    if (items.length === 0) {
    ...
    items.forEach((item) => { 
        ...
        input.type = 'checkbox'; input.id = checkboxId; input.name = groupName; input.value = item;
        ...
        label.htmlFor = checkboxId; label.textContent = item; 
        label.className = 'ml-2 block text-sm text-gray-900';
        div.appendChild(input); div.appendChild(label); container.appendChild(div);
    });
}

function populateRecycleView(rvContainer: HTMLElement | null, itemsArray: string[], listId: string): void {
    if (!rvContainer) {
        console.error(`populateRecycleView ('${listId}'): CRITICAL - Contenedor de lista (ID: ${listId}) NO ENCONTRADO.`);
        if (statusLabel) updateStatus(`Error: Contenedor para lista '${listId}' no encontrado.`);
        return;
    }
        ...
        itemsArray.forEach((itemText, index) => {
            ...
            listItem.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    handleListItemSelection(e as unknown as MouseEvent); 
                }
            });
            rvContainer.appendChild(listItem);
        });
        if(rvContainer.parentElement) rvContainer.parentElement.setAttribute('aria-live', 'polite');
    }
    updateListCounters();
}

// --- Lógica de Selección de Items en Listas ---
function handleListItemSelection(event: MouseEvent | KeyboardEvent): void {
...
function movePreferenceItem(listId: 'rvRegiones' | 'rvIdiomas', direction: 'up' | 'down'): void {
    ...
    if (!selectedItemElement || selectedItemElement.dataset.listId !== listId) {
        showCustomMessage(`Selecciona un ítem de la lista '${listNameForUI}' para mover.`, 'info'); return;
    }
    ...
    else if (direction === 'down' && currentIndex < itemsArrayRef.length - 1) newIndex = currentIndex + 1;
    else return; 
    ...
    if (container) {
        const newItemsInDOM = container.querySelectorAll<HTMLElement>('.list-item');
    ...
    if (file && filePathDisplay && statusLabel) {
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
            try {
                ...
                if (btnPrevisualizar) btnPrevisualizar.disabled = false; 
                if (btnGenerarFinal) btnGenerarFinal.disabled = true;
                ...
                if(filePathDisplay) filePathDisplay.textContent = "Error al procesar el archivo."; romFileContent = null;
                if (btnPrevisualizar) btnPrevisualizar.disabled = true;
            }
        };
        reader.onerror = () => {
            showCustomMessage(`Error al leer el archivo: ${reader.error?.message || 'Error desconocido del lector'}`, 'error');
            if (filePathDisplay) filePathDisplay.textContent = "Error al leer el archivo."; romFileContent = null;
            if (btnPrevisualizar) btnPrevisualizar.disabled = true;
        }
        reader.readAsText(file);
    } else {
        if(filePathDisplay) filePathDisplay.textContent = "Ningún archivo seleccionado."; romFileContent = null;
        if (btnPrevisualizar) btnPrevisualizar.disabled = true;
    }
}

// --- Actualizar Configuración desde UI ---
function actualizarConfigDesdeUI(): void {
    console.log("actualizarConfigDesdeUI: Actualizando currentConfig desde la UI...");
    if (tagsCheckboxesContainer) {
        tagsCheckboxesContainer.querySelectorAll<HTMLInputElement>('input[type="checkbox"]').forEach(chk => { 
    ...
    if (regionesExcluirCheckboxesContainer) {
        regionesExcluirCheckboxesContainer.querySelectorAll<HTMLInputElement>('input[type="checkbox"]').forEach(chk => { 
             const itemName = chk.dataset.itemName;
...
async function previsualizarFiltro(): Promise<void> {
    if (!romFileContent || romFileContent.length === 0) {
        showCustomMessage("Por favor, selecciona y carga un archivo de ROMs con contenido.", 'error'); return;
    }
    updateStatus("Procesando previsualización...");
    if (!btnPrevisualizar) { console.error("previsualizarFiltro: btnPrevisualizar no encontrado."); return; }
    ...
    try {
        ...
        if (btnGenerarFinal) btnGenerarFinal.disabled = (simulacionResultados.salvados?.length || 0) === 0;
...
function moveRomBetweenLists(sourceListId: 'salvados' | 'eliminados', targetListId: 'salvados' | 'eliminados'): void {
    ...
    if (!selectedItemElement || selectedItemElement.dataset.listId !== sourceListId) {
        showCustomMessage(`Selecciona un ROM de la lista '${sourceListName}' para mover.`, 'info'); return;
    }
    const romName = selectedItemElement.dataset.id!; 
    let sourceArray: string[], targetArray: string[], sourceContainer: HTMLElement | null, targetContainer: HTMLElement | null;

    if (sourceListId === 'salvados') { 
        sourceArray = simulacionResultados.salvados; targetArray = simulacionResultados.eliminados; 
        sourceContainer = rvSalvadosContainer; targetContainer = rvEliminadosContainer; 
    } else { 
        sourceArray = simulacionResultados.eliminados; targetArray = simulacionResultados.salvados; 
        sourceContainer = rvEliminadosContainer; targetContainer = rvSalvadosContainer; 
    }
    ...
    if (indexInSource > -1) {
        ...
        if (targetContainer) {
            const newItemsInDOMTarget = targetContainer.querySelectorAll<HTMLElement>(`.list-item[data-id="${CSS.escape(romName)}"]`);
...
function generarArchivoFinal(): void {
    if (!simulacionResultados.salvados || simulacionResultados.salvados.length === 0) {
        showCustomMessage("No hay ROMs en la lista de 'Salvados'. El archivo no se generará.", 'warning'); return;
    }
...
function updateStatus(message: string): void {
    ...
    else console.warn("updateStatus: statusLabel no encontrado. Mensaje:", message);
}

function updateListCounters(): void {
    if (labelSalvados) labelSalvados.textContent = `ROMs a Salvar (${simulacionResultados.salvados?.length || 0}):`;
    if (labelEliminados) labelEliminados.textContent = `ROMs a Eliminar (${simulacionResultados.eliminados?.length || 0}):`;
}

function showCustomMessage(message: string, type: 'info' | 'success' | 'error' | 'warning' = 'info'): void {
    ...
    if (type === 'success') customMessageBox.classList.add('success');
    else if (type === 'error') customMessageBox.classList.add('error');
    else if (type === 'warning') { 
      customMessageBox.classList.remove('info', 'success', 'error'); 
      customMessageBox.classList.add('warning'); 
    }
    else customMessageBox.classList.add('info');
...
function initializeUI(): void {
    ...
    if (btnGenerarFinal) btnGenerarFinal.disabled = (simulacionResultados.salvados?.length || 0) === 0; 
    else console.error("initializeUI: btnGenerarFinal no encontrado.");
    
    if (btnPrevisualizar) btnPrevisualizar.disabled = !romFileContent || romFileContent.length === 0;
    else console.error("initializeUI: btnPrevisualizar no encontrado.");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnPrevisualizar = document.getElementById('btnPrevisualizar') as HTMLButtonElement | null;
    ...
    btnGenerarFinal = document.getElementById('btnGenerarFinal') as HTMLButtonElement | null;
    ...
    function checkElem(id: string, elem: HTMLElement | null): boolean {
        if (!elem) {
    ...
    if (!essentialElementsFound) {
        ...
        if (statusLabel) updateStatus(baseErrorMsg); else console.error(baseErrorMsg);
        alert(baseErrorMsg); 
        return; 
    }
    ...
    if (btnPrevisualizar) btnPrevisualizar.addEventListener('click', previsualizarFiltro);
    if (btnGenerarFinal) btnGenerarFinal.addEventListener('click', generarArchivoFinal);
    
    if (btnRegionUp) btnRegionUp.addEventListener('click', () => movePreferenceItem('rvRegiones', 'up'));
    if (btnRegionDown) btnRegionDown.addEventListener('click', () => movePreferenceItem('rvRegiones', 'down'));
    if (btnIdiomaUp) btnIdiomaUp.addEventListener('click', () => movePreferenceItem('rvIdiomas', 'up'));
    if (btnIdiomaDown) btnIdiomaDown.addEventListener('click', () => movePreferenceItem('rvIdiomas', 'down'));

    if (btnMoverAEliminados) btnMoverAEliminados.addEventListener('click', () => moveRomBetweenLists('salvados', 'eliminados'));
    if (btnMoverASalvados) btnMoverASalvados.addEventListener('click', () => moveRomBetweenLists('eliminados', 'salvados'));
    
    if(customMessageButton && customMessageBox) customMessageButton.addEventListener('click', () => { if(customMessageBox) customMessageBox.style.display = 'none'; });
